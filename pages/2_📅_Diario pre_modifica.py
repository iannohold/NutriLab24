...
if not st.session_state.get("logged_in"):
    st.warning("⚠️ Effettua il login dalla pagina principale per accedere.")
    st.stop()
...
# ==========================================
# 📊 UI HELPERS (Stack Bar: Consumato vs Pianificato)
# ==========================================
def render_stacked_prog(col, label, consumato, pianificato, tgt, unit):
    ...
    with col:
        if tgt > 0:
            ...
            # Calcolo differenze reali e ipotetiche
            diff_cons = tgt - consumato
            diff_tot = tgt - totale
            
            # Etichette testuali superiori: Consumato (Totale previsto) / Target
            if pianificato > 0:
            ...
            # Barra di progresso a due colori (Blu = Consumato, Grigio = Pianificato)
            st.markdown(f"""
            ...
            # Sottotitolo "Mancano:" con logica separata
            if diff_cons >= 0:
                if pianificato > 0:
                    if diff_tot >= 0:
...
with tab_inserisci:
    ...
    if not df_giorno_sel.empty:
        # SEPARIAMO CONSUMATO VS PIANIFICATO
        df_consumati = df_giorno_sel[df_giorno_sel['Stato'] == 'Consumato']
        df_pianificati = df_giorno_sel[df_giorno_sel['Stato'] == 'Pianificato']
        
        # Totali reali (solo consumati)
        t_cal = df_consumati['Calorie'].sum()
        t_c = df_consumati['Carboidrati'].sum()
        t_p = df_consumati['Proteine'].sum()
        t_f = df_consumati['Grassi'].sum()

        # Totali pianificati (da mostrare a parte in grigio)
        p_cal = df_pianificati['Calorie'].sum()
        p_c = df_pianificati['Carboidrati'].sum()
        p_p = df_pianificati['Proteine'].sum()
        p_f = df_pianificati['Grassi'].sum()

        # Visualizzazione Barre Stacked (Consumato + Pianificato)
        if tgt_cal > 0:
        ...
        for pasto in ["Colazione", "Spuntino", "Pranzo", "Merenda", "Cena"]:
            ...
            if not df_pasto.empty:
                ...
                with st.expander(f"{alert_icon}🍽️ {pasto.upper()} (Totale blocco: {t_cal_p:.0f} kcal)", expanded=has_pianificati):
                    for _, row in df_pasto.iterrows():
                        ...
                        if is_pianificato:
                            # STILE GRIGIO per i pasti non confermati
                        ...
                        if st.session_state.get(f"editing_{row['ID']}", False):
                            with st.form(key=f"form_edit_{row['ID']}"):
                                ...
                                new_elem = st.text_input("Nome Elemento", value=row['Elemento'])
                                new_cal = st.number_input("Calorie", value=float(row['Calorie']))
                                new_p = st.number_input("Proteine (g)", value=float(row['Proteine']))
                                new_c = st.number_input("Carboidrati (g)", value=float(row['Carboidrati']))
                                new_f = st.number_input("Grassi (g)", value=float(row['Grassi']))
                                
                                if st.form_submit_button("💾 Salva Modifiche"):
                                    # MODIFICA VIA SQL
                                    aggiorna_voce_diario(
                                        id_voce=row['ID'], 
                                        nuova_qta=float(row['Quantita']), 
                                        cal=new_cal, 
                                        c=new_c, 
                                        p=new_p, 
                                        f=new_f, 
                ...
                for _, row in df_g.iterrows():
                    ...
                    if st.session_state.get(f"editing_{row['ID']}", False):
                        with st.form(key=f"form_edit_stor_{row['ID']}"):
                            ne = st.text_input("Elemento", value=row['Elemento'], key=f"ne_{row['ID']}")
                            nc = st.number_input("Calorie", value=float(row['Calorie']), key=f"nc_{row['ID']}")
                            if st.form_submit_button("Salva"):
                                # MODIFICA STORICO VIA SQL
                                aggiorna_voce_diario(
                                    id_voce=row['ID'],
                                    nuova_qta=float(row['Quantita']),
                                    cal=nc,
                                    c=float(row['Carboidrati']),
                                    p=float(row['Proteine']),
                                    f=float(row['Grassi']),
...
with tab_report:
    ...
    if not df_rep_base.empty:
        ...
        if not df_rep.empty:
            ...
            # Dividiamo in Consumati e Pianificati per la media
